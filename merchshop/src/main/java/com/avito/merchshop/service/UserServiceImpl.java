...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<SendCoinResponse> sendCoin(SendCoinRequest sendCoinRequest, String token) {
        try {
            ...
            userRepository.updateAmountByUser(userFrom.getUsername(), userFrom.getCoins() - sendCoinRequest.getAmount());
            userRepository.updateAmountByUser(userTo.getUsername(), userTo.getCoins() + sendCoinRequest.getAmount());

            Transaction transaction = transactionRepository.findBySenderAndReceiver(userFrom.getUsername(), userTo.getUsername());
            if (transaction == null) {
                ...
                transactionRepository.updateAmountBySenderAndReceiver(transaction.getSender(), transaction.getReceiver(), transaction.getAmount());
            }
    ...
    public ResponseEntity<BuyItemResponse> buyItem(String itemName, String token) {
        try {
            ...
            Inventory inventory = inventoryRepository.findByOwnerAndItem(user.getUsername(), item.getItem());
            if (inventory == null) {
                ...
                inventoryRepository.updateAmountByOwnerAndItem(user.getUsername(), item.getItem(), inventory.getAmount() + 1);
            }
