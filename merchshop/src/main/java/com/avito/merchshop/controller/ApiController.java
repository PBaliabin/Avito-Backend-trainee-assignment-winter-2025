...
public class ApiController {
    ...
    public ResponseEntity<SendCoinResponse> sendCoin(@RequestBody SendCoinRequest sendCoinRequest, @RequestHeader("Authorization") String token) {
        return userService.sendCoin(sendCoinRequest, token);
    }

    @GetMapping("/buy/{item}")
    public ResponseEntity<BuyItemResponse> buyItem(@PathVariable String item, @RequestHeader("Authorization") String token) {
