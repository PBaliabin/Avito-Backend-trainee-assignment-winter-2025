...
class UserServiceImplTest {
  ...
  @MockBean private JwtTokenService jwtTokenService; // Настоящий Jwt сервис

  @MockBean private UserRepository userRepository; // Мок базы данных

  @MockBean private InventoryRepository inventoryRepository; // Мок инвентаря

  @MockBean private TransactionRepository transactionRepository; // Мок транзакций

  @MockBean private ItemRepository itemRepository; // Мок товаров
  ...
  void buyItem_SuccessfulPurchase() {
    ...
    when(inventoryRepository.findByOwnerAndItem(user.getUsername(), item.getItem()))
        .thenReturn(null);
  ...
  void buyItem_InsufficientFunds_ThrowsBadRequestException() {
    ...
    assertThrows(
        BadRequestException.class, () -> userService.buyItem(expensiveItem.getItem(), token));
  }
